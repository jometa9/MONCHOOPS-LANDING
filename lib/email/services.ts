...
export async function sendSubscriptionChangeEmail({
  ...
  try {
    ...
    let subject = "Your MonchoOps subscription has been updated";
    if (status === "active") {
      subject = "Your MonchoOps subscription is active";
    } else if (status === "trialing") {
      subject = "Your MonchoOps trial has started";
    } else if (status === "canceled") {
      subject = "Your MonchoOps subscription has been canceled";
    } else if (status === "canceling") {
      subject = "Your MonchoOps subscription cancellation has been scheduled";
    } else if (status === "plan_changed") {
      subject = "Your MonchoOps plan has been changed";
    } else if (status === "unpaid") {
      subject = "There was a payment issue with your MonchoOps subscription";
    } else if (status === "expired") {
      subject = "Your MonchoOps subscription has expired";
    }
...
export async function sendPasswordResetEmail({
  ...
  return sendEmail({
    to: email,
    subject: "Reset password for your MonchoOps account",
    html,
    text,
  });
}
...
export async function sendVersionUpdateEmail({
  ...
  const subject = isCritical
    ...
    : `New version ${newVersion} available for MonchoOps`;
