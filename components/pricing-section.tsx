...
interface UserEntitlements {
  monchoops: ProductSubscription | null;
}
...
export function PricingSection({
  ...
  const [entitlements, setEntitlements] = useState<UserEntitlements>({
    monchoops: contextData?.entitlements.monchoops || null,
  });
  const [priceIds, setPriceIds] = useState<{
    monchoops: ProductPrices;
  }>({
    monchoops: {
      proMonthly: null,
      proAnnual: null,
  ...
  const handleDownloadClick = async () => {
    await handleDownload("monchoops", userOS);
  };
  ...
  const handleManageSubscription = async () => {
    ...
    try {
      const result = await customerPortalAction("monchoops");
      if (result?.redirect) {
  ...
  const handleCheckout = async (plan: string, priceId: string | null) => {
    ...
    const productKey: ProductKey = "monchoops";
    ...
    try {
      ...
      if (isUpgrade || isBillingPeriodChange) {
        ...
        const result = await changePlanAction("monchoops", priceId);

        if (result.success) {
          window.location.href = `/dashboard/pricing?success=plan-changed&product=monchoops`;
          return;
        }
  ...
  const currentPrices = priceIds.monchoops;
  const currentSubscription = entitlements.monchoops;
