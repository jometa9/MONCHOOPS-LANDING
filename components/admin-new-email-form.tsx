  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/inbox/${replyToEmailId}`)
      ...
      .then((data) => {
        if (data?.email) {
          setOriginalEmail({
            subject: data.email.subject || t("noSubject"),
        ...
        {sendEmailStatus === "success" && (
          <div className="rounded-lg bg-white border border-gray-200 p-3">
            <p className="text-sm text-gray-600">
              {lastSentCount === 1 && lastFailedCount === 0 && t("emailSentOk")}
              {lastSentCount > 1 && lastFailedCount === 0 && t("emailsSentOk", { count: lastSentCount })}
              {lastFailedCount > 0 && t("emailsSentMixed", { sent: lastSentCount, failed: lastFailedCount })}
            </p>
          </div>
        )}
